...
def train(config, ds_path):
    ...
    out_project = str(PROJECT_ROOT / out.get("project", "runs/yolov11"))

    print(f"\nMemuat model: {config.get('model', 'yolo11n.pt')}")
...
def main():
    ...
    # Summary
    t = config.get("training", {})
    print(f"  Model     : {config.get('model', 'yolo11n.pt')}")
    ...
    if best.exists():
        r = input("\nCopy best.pt ke models/yolo v11? (y/n): ").strip().lower()
        if r in ("y", "yes", ""):
            dst = PROJECT_ROOT / "models" / "yolo v11"
